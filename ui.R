...
fluidPage(
  ...
  titlePanel(
    wellPanel(
      fluidRow(
        ...
        column(6, align="center", tags$b(h2("ODWC Data Validation Application")),
               ...
               hr(), 
               h5("Created by OCRAP Subcommittee")),
    ...
    windowTitle = "OK Creel Analysis App" #this text is what appears as browser title
  ),
  tabsetPanel(type = c("tabs"),
              
              
    #Validate Pressure Data Tab####
    tabPanel("Validate Pressure Count Data",
      hr(),
      fluidRow(
        column(5,
          sidebarPanel(width = 12,
            ...
            h4(tags$b("1) Upload Pressure data file (should only include one combination of gear and lake at a time)")),
            
            helpText("Instructions and considerations for Pressure data entry are available in 
                     Data Entry Instructions Tab"),
            
            fileInput("PressureData", "Upload pressure Count Data"),
            ...
            helpText("Upload file (.csv) must have correct format and column headings. To see proper columns click on Dowload Template 
                     button below"),
            downloadButton("PressureCount", label = "Download Template",class="butnTmplt"),
            
            HTML('<br/>', '<br/>'),
            #checkboxInput("period2NA", "Change Periods to NA's", value = FALSE), #decided to hard code this
            hr(),
            h4(tags$b("2) Initiate pressure data validation check")),
              actionButton("validateCount", "Initiate Validation", class="butn"),
            ...
            hr(),
            h4(tags$b("4) Download validated Pressure data")),
            helpText("If all error rules are 'Okay', dataset is ready to download."),
            helpText("Validated data will have required PressureID field and will be ready to
                     upload to main app."),
            
            
            h4(textOutput("downloadMessage")),
            downloadButton("valPressureData", "Download validated Pressure data", class="butn", disabled = "disabled")
          )  
        ),
        ...
        column(3, align = "center",
          wellPanel(
            ...
            formattableOutput("PressureError")
          )
        ),
      ...
      fluidRow(
        ...
        # DT::dataTableOutput("PressureDataDisplayTable")
        DT::DTOutput("PressureDataDisplayTable")
      )
    ),
    
    
    
    #Validate Creel Data Tab####
    tabPanel("Validate Creel Data",
      hr(),
      fluidRow(
        column(5,
          sidebarPanel(width = 12,
            h4(tags$b("1) Upload creel data file")),
            helpText("Instructions and considerations for sample data entry are available in 
                     Data Entry Instructions Tab"),
            fileInput("creelData", "Upload Fish Creel Data"),
            ...
            downloadButton("CreelTemplate", label = "Download Template",class="butnTmplt"),
            HTML('<br/>', '<br/>'),
            
            #checkboxInput("creelperiod2NA", "Change Periods to NA's", value = FALSE), #decided to hard code this
            hr(),
            h4(tags$b("2) Initiate creel data validation check")),
            actionButton("validatecreel", "Initiate Validation",class="butn"),
            helpText("Error Identification Table will appear in middle panel."),
            hr(),
            h4(tags$b("3) Observe error status and fix as needed")),
            helpText("Fix any errors identified by the data validation function (Error Identification Table).
            ...
            helpText("Use 3rd panel to identify specific row numbers violating each error rule."),
            hr(),
            h4(tags$b("4) Download validated creel data")),
            helpText("If all error rules are 'Okay', creel dataset is ready to download."),
            helpText("Validated creel data will be ready to
                     upload to main app."),
            
            
            h4(textOutput("downloadMessageCreel")),
            downloadButton("valCreelData", "Download validated creel data", class="butn", disabled = "disabled")
          )  
        ),
        column(3, align = "center",
           wellPanel(
